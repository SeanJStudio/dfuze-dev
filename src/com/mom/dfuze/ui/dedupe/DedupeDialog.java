...
public class DedupeDialog extends JDialog {
	...
	private DedupeViewDialog dedupeViewDialog;
	...
	static final String FIX_APT_REGEX = "((trlr|trailer|lot|apartment|appartement|suite|suit|ste|spc|space|room|rm|office|ofc|unit|bureau|piece|(?<=^|\\s+)ph|(?<=^|\\s+)th)(?=\\s+|$|#|-|\\d+))";
	...
	public DedupeDialog(JFrame frame) {
		super(frame, true);
		...
		this.btnView.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					if (e.getSource() == DedupeDialog.this.btnView) {
						LinkedHashSet<String> selectedFields = new LinkedHashSet<>();
						if (DedupeDialog.this.comboBoxName1.getSelectedIndex() > -1)
							selectedFields.add(DedupeDialog.this.comboBoxName1.getSelectedItem().toString()); 
						if (DedupeDialog.this.comboBoxName2.getSelectedIndex() > -1)
							selectedFields.add(DedupeDialog.this.comboBoxName2.getSelectedItem().toString()); 
						if (DedupeDialog.this.comboBoxIfName1Blank.getSelectedIndex() > -1)
							selectedFields.add(DedupeDialog.this.comboBoxIfName1Blank.getSelectedItem().toString()); 
						if (DedupeDialog.this.comboBoxIfName2Blank.getSelectedIndex() > -1)
							selectedFields.add(DedupeDialog.this.comboBoxIfName2Blank.getSelectedItem().toString()); 
						if (DedupeDialog.this.comboBoxPriority.getSelectedIndex() > -1)
							selectedFields.add(DedupeDialog.this.comboBoxPriority.getSelectedItem().toString()); 
						DedupeDialog.this.dedupeViewDialog = new DedupeViewDialog(DedupeDialog.this, DedupeDialog.this.allDupeRecordsList, (String[])selectedFields.toArray((Object[])new String[0]));
						DedupeDialog.this.dedupeViewDialog.setDefaultCloseOperation(2);
						DedupeDialog.this.dedupeViewDialog.setModalityType(Dialog.ModalityType.APPLICATION_MODAL);
						DedupeDialog.this.dedupeViewDialog.setVisible(true);
					} 
				} catch (Exception err) {
		...
		this.btnRun.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == DedupeDialog.this.btnRun) {
					if(UiController.getUserData().getRecordList().size() == 0) {
						...
						try {
							...
							Runnable runner = new Runnable() {
								public void run() {
									...
									try {
										...
										dedupe(UiController.getUserData());
							...
							DedupeDialog.this.dedupeThread = new Thread(runner, "Code Executer");
							DedupeDialog.this.dedupeThread.start();
	...
	public void dedupe(UserData userData) throws ApplicationException {
		...
		try {
			...
			if (this.chckbxKillFilePriority.isSelected())
				if (!this.isDfPrioritySet) {
					...
					//this.killPriority = ((Record)userData.getRecordList().get(0)).getPriority();
				}  
			//String[][] prefixesFirstPass = TextReader.readPrefixPasses(new File("prefixes_first_pass.csv"), ',');
			//String[][] prefixesSecondPass = TextReader.readPrefixPasses(new File("prefixes_second_pass.csv"), ',');
			...
			preprocessAddresses(sortedArray);
			Collections.sort(sortedArray, (Comparator<? super Record>)new RecordSorters.CompareByDupePCodeDupeAdd1DupeAdd2());
			...
			if (this.rdbtnDedupeAddress.isSelected() || this.rdbtnDedupePC.isSelected()) {
				for (int i = 0; i < sortedArray.size(); i++) {
					this.lblStatus.setText(String.format("Searching for duplicate addresses on record %d / %d", new Object[] { Integer.valueOf(i + 1), Integer.valueOf(sortedArray.size()) }));
					...
					if (currentDupesToAdd.size() > 0) {
						...
						if (!isNameSelected && !this.chckbxKillFilePriority.isSelected()) {
							this.lblParentDupesAmount.setText(String.valueOf(Integer.parseInt(this.lblParentDupesAmount.getText()) + 1));
							this.lblChildDupesAmount.setText(String.valueOf(Integer.parseInt(this.lblChildDupesAmount.getText()) + currentDupesToAdd.size() - 1));
							this.lblTotalDupesAmount.setText(String.valueOf(Integer.parseInt(this.lblTotalDupesAmount.getText()) + currentDupesToAdd.size()));
						} 
					} 
									...
									for (int l = dupeRecordGroups.size() - 1; l >= 0; l--) {
										...
										for (int k = recordList.size() - 1; k >= 0; k--) {
											if (((Record)recordList.get(k)).getDfId() == ((Record)sortedArray.get(i)).getDfId() && 
												...
												if (!isNameSelected && !this.chckbxKillFilePriority.isSelected()) {
													this.lblChildDupesAmount.setText(String.valueOf(Integer.parseInt(this.lblChildDupesAmount.getText()) + 1));
													this.lblTotalDupesAmount.setText(String.valueOf(Integer.parseInt(this.lblTotalDupesAmount.getText()) + 1));
												} 
											} 
										} 
									} 
				...
				for (List<Record> recordList : dupeRecordGroups) {
					...
					if (!this.chckbxKillFilePriority.isSelected())
						for (int i = 0; i < recordList.size(); i++) {
							...
							this.allDupeRecordsList.add(recordList.get(i));
						}  
					if (this.chckbxKillFilePriority.isSelected()) {
						...
						if (tempKillDupeArray.size() > 1) {
							for (i = 0; i < tempKillDupeArray.size(); i++) {
								...
								this.allDupeRecordsList.add(tempKillDupeArray.get(i));
							} 
							this.lblChildDupesAmount.setText(String.valueOf(Integer.parseInt(this.lblChildDupesAmount.getText()) + tempKillDupeArray.size() - 1));
							this.lblTotalDupesAmount.setText(String.valueOf(Integer.parseInt(this.lblTotalDupesAmount.getText()) + tempKillDupeArray.size()));
						} 
					} 
				}  
			if (isNameSelected) {
				...
				findNameDupes(dupeRecordGroups);
			} 
			...
			addExtraDupeInfoStepTwo(allDupeRecordsList); // Fixes wrong group sizes and random groupIds
			
			// Remove dupe info from records that may not have met the name criteria
			Set<Integer> idHashSet = new HashSet<>();
			
			for(Record dupeRecord : allDupeRecordsList)
			...
			this.lblStatus.setText("Complete");
		} 
	}
	...
	private void preprocessAddresses(List<Record> recordList) throws ApplicationException {
		...
		if (this.rdbtnDedupeAddress.isSelected() || this.rdbtnDedupePC.isSelected()) {
			...
			for (int i = 0; i < recordList.size(); i++) {
				this.progressBar.setValue(this.progressBar.getValue() + 1);
				this.lblStatus.setText(String.format("Pre-processing address for record %d / %d", new Object[] { Integer.valueOf(i), Integer.valueOf(recordList.size()) }));
				Record record = recordList.get(i);
	...
	private void findNameDupes(ArrayList<List<Record>> dupeAddressGroups) throws Exception {
		...
		if (dupeAddressGroups.size() > 0) {
			...
			this.progressBar.setMaximum(totalRecords);
			this.progressBar.setValue(0);
			for (List<Record> list : dupeAddressGroups) {
				for (int i = 0; i < list.size(); i++) {
					...
					List<Record> currentDupesToAdd = new ArrayList<>();
					this.progressBar.setValue(this.progressBar.getValue() + 1);
					this.lblStatus.setText(String.format("Searching for name dupes on record %d / %d possible dupes", new Object[] { Integer.valueOf(++recordProcessed), Integer.valueOf(totalRecords) }));
					boolean dupeWasFound = false;
					int startIndex = i + 1;
	...
	private void preprocessNames(ArrayList<List<Record>> dupeRecordGroups, Pattern prefixPattern) {
		if (dupeRecordGroups.size() > 0) {
			...
			for (List<Record> recordList : dupeRecordGroups) {
				for (int i = 0; i < recordList.size(); i++) {
					this.progressBar.setValue(this.progressBar.getValue() + 1);
					this.lblStatus.setText(String.format("Pre-processing name for record %d of %d possible duplicates", new Object[] { Integer.valueOf(++processedRecord), Integer.valueOf(totalRecords) }));
					Record record = recordList.get(i);
			...
			try {
				if (e.getSource() instanceof JComboBox) {
					...
					if(userComboBox.getSelectedIndex() > -1) {
						DedupeDialog.this.addToMappedFields(userComboBox);

						if (e.getSource() == DedupeDialog.this.comboBoxName1 || e.getSource() == DedupeDialog.this.comboBoxName2)
							DedupeDialog.this.btnRun.setEnabled(true); 
					}
				} 
			} catch (Exception err) {
				UiController.handle(err);
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() instanceof JComboBox) {
					...
					if (userComboBox.getSelectedIndex() > -1) {
						DedupeDialog.this.lblPrioritySorting.setEnabled(true);
						DedupeDialog.this.rdbtnAsc.setEnabled(true);
						DedupeDialog.this.rdbtnDesc.setEnabled(true);
						DedupeDialog.this.lblPriorityFormat.setEnabled(true);
						DedupeDialog.this.comboBoxPriorityFormat.setEnabled(true);

						if (!DedupeDialog.this.rdbtnDedupeName.isSelected())
							DedupeDialog.this.btnRun.setEnabled(true); 
						if (DedupeDialog.this.rdbtnDedupeName.isSelected() && (DedupeDialog.this.comboBoxName1.getSelectedIndex() > -1 || DedupeDialog.this.comboBoxName2.getSelectedIndex() > -1))
							DedupeDialog.this.btnRun.setEnabled(true); 
						if (DedupeDialog.this.comboBoxPriorityFormat.getSelectedItem().toString().equalsIgnoreCase(DedupeDialog.PRIORITY_FORMATS.DATE.toString())) {
							DedupeDialog.this.lblPriorityDateFormat.setEnabled(true);
							DedupeDialog.this.textFieldPriorityDateFormat.setEnabled(true);
							DedupeDialog.this.btnPriorityDateFormatHelp.setEnabled(true);	
						}


					} else if (userComboBox.getSelectedIndex() == -1) {
						DedupeDialog.this.lblPrioritySorting.setEnabled(false);
						DedupeDialog.this.rdbtnAsc.setEnabled(false);
						DedupeDialog.this.rdbtnDesc.setEnabled(false);
						DedupeDialog.this.lblPriorityFormat.setEnabled(false);
						DedupeDialog.this.comboBoxPriorityFormat.setEnabled(false);
						DedupeDialog.this.btnRun.setEnabled(false); 
						DedupeDialog.this.btnRun.setEnabled(false); 
						DedupeDialog.this.lblPriorityDateFormat.setEnabled(false);
						DedupeDialog.this.textFieldPriorityDateFormat.setEnabled(false);
						DedupeDialog.this.btnPriorityDateFormatHelp.setEnabled(false);			
					}
				} 
			} catch (Exception err) {
				UiController.handle(err);
	...
	private class DedupeRadioButtonHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() == DedupeDialog.this.rdbtnDedupeName)
					if (DedupeDialog.this.comboBoxName1.getSelectedIndex() <= -1 && DedupeDialog.this.comboBoxName2.getSelectedIndex() <= -1) {
						DedupeDialog.this.btnRun.setEnabled(false);
					} else {
						DedupeDialog.this.btnRun.setEnabled(true);
					}
				
				if (e.getSource() == DedupeDialog.this.rdbtnDedupeAddress || e.getSource() == DedupeDialog.this.rdbtnDedupePC) {
					DedupeDialog.this.btnRun.setEnabled(true);
					DedupeDialog.this.chckbxConvertPcToFsa.setEnabled(true);
				} else {
					DedupeDialog.this.chckbxConvertPcToFsa.setEnabled(false);
					DedupeDialog.this.chckbxConvertPcToFsa.setSelected(false);
				}
				
				if (e.getSource() == DedupeDialog.this.rdbtnDedupeAddress) {
					DedupeDialog.this.chckbxIgnoreApt.setEnabled(true);
				} else {
					DedupeDialog.this.chckbxIgnoreApt.setEnabled(false);
					DedupeDialog.this.chckbxIgnoreApt.setSelected(false);
				} 
			} catch (Exception err) {
				UiController.handle(err);
			} 
	...
	private class DedupeChckbxExactNamesOnlyHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() == DedupeDialog.this.chckbxExactNamesOnly) {
					if (DedupeDialog.this.chckbxExactNamesOnly.isSelected()) {
						DedupeDialog.this.textFieldNameSimilarityPercentage.setEnabled(false);
						DedupeDialog.this.lblPercentageForNameSimilarity.setEnabled(false);
						DedupeDialog.this.labelPercentage.setEnabled(false);
						DedupeDialog.this.chckbxMatchOnNameSimilarityOnly.setEnabled(false);
					} 
					if (!DedupeDialog.this.chckbxExactNamesOnly.isSelected()) {
						DedupeDialog.this.textFieldNameSimilarityPercentage.setEnabled(true);
						DedupeDialog.this.lblPercentageForNameSimilarity.setEnabled(true);
						DedupeDialog.this.labelPercentage.setEnabled(true);
						DedupeDialog.this.chckbxMatchOnNameSimilarityOnly.setEnabled(true);
					} 
				} 
			} catch (Exception err) {
				UiController.handle(err);
	...
	private class DedupeChckbxMatchOnNameSimilarityOnlyHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() == DedupeDialog.this.chckbxMatchOnNameSimilarityOnly) {
					if (DedupeDialog.this.chckbxMatchOnNameSimilarityOnly.isSelected())
						DedupeDialog.this.chckbxExactNamesOnly.setEnabled(false); 
					if (!DedupeDialog.this.chckbxMatchOnNameSimilarityOnly.isSelected())
						DedupeDialog.this.chckbxExactNamesOnly.setEnabled(true); 
				} 
			} catch (Exception err) {
				UiController.handle(err);
			} 
	...
	private class DedupeChkbxKillFileHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() == DedupeDialog.this.chckbxKillFilePriority)
					if (DedupeDialog.this.chckbxKillFilePriority.isSelected())
						if (DedupeDialog.this.comboBoxPriority.getSelectedIndex() == -1) {
							DedupeDialog.this.btnRun.setEnabled(false);
						} else if (!DedupeDialog.this.rdbtnDedupeName.isSelected()) {
							DedupeDialog.this.btnRun.setEnabled(true);
						} else if (DedupeDialog.this.comboBoxName1.getSelectedIndex() > -1 || DedupeDialog.this.comboBoxName2.getSelectedIndex() > -1) {
							DedupeDialog.this.btnRun.setEnabled(true);
						}   
				if (!DedupeDialog.this.chckbxKillFilePriority.isSelected())
					if (!DedupeDialog.this.rdbtnDedupeName.isSelected()) {
	...
	private class ComboBoxPriorityFormatHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() == DedupeDialog.this.comboBoxPriorityFormat)
					if (DedupeDialog.this.comboBoxPriorityFormat.getSelectedItem().toString().equalsIgnoreCase(DedupeDialog.PRIORITY_FORMATS.DATE.toString())) {
						DedupeDialog.this.lblPriorityDateFormat.setEnabled(true);
						DedupeDialog.this.textFieldPriorityDateFormat.setEnabled(true);
						DedupeDialog.this.btnPriorityDateFormatHelp.setEnabled(true);
	...
	private class ExportButtonHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() == DedupeDialog.this.btnExport && 
						DedupeDialog.this.allDupeRecordsList.size() > 0) {
					DedupeDialog.this.dedupeExportDialog = new DedupeExportDialog(DedupeDialog.this, DedupeDialog.this.allDupeRecordsList);
					DedupeDialog.this.dedupeExportDialog.setVisible(true);
				} 
			} catch (Exception err) {
				UiController.handle(err);
			} 
	...
	private class PriorityDateFormatHelpButtonHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == DedupeDialog.this.btnPriorityDateFormatHelp) {
				...
				JOptionPane.showMessageDialog(DedupeDialog.this, editorPane, "Date formats", 1);
			} 
		}
	}
	...
	private class DedupeOptionHelpButtonHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == DedupeDialog.this.btnDedupeOptionHelp) {
				JEditorPane editorPane = new JEditorPane("text/html", 
				...
				editorPane.setEditable(false);
				JOptionPane.showMessageDialog(DedupeDialog.this, editorPane, "Find dedupe options", 1);
			} 
		}
	}
	...
	private class DeleteButtonHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				if (e.getSource() == DedupeDialog.this.btnDelete && 
						DedupeDialog.this.allDupeRecordsList.size() > 0) {
					int buttonPressed = JOptionPane.showConfirmDialog(DedupeDialog.this, "Would you like to export before deleting?", "Delete Duplicates", 
							1, 3);
					if (buttonPressed == 0) {
						DedupeDialog.this.dedupeExportDialog = new DedupeExportDialog(DedupeDialog.this, DedupeDialog.this.allDupeRecordsList);
						DedupeDialog.this.dedupeExportDialog.setVisible(true);
					} 
					if (buttonPressed == 1 || (DedupeDialog.this.dedupeExportDialog != null && DedupeDialog.this.dedupeExportDialog.getIsNextPressed())) {
						Runnable runner = new Runnable() {
							public void run() {
								try {
									disableUI();
	...
	private class WindowClosingHandler extends WindowAdapter {
		...
		public void windowClosing(WindowEvent e) {
			DedupeDialog.this.isDialogClosed = true;
		...
		public void keyPressed(KeyEvent e) {
			...
			if (key == 127 || key == 8) {
				if(e.getSource() instanceof JComboBox) {
					...
					if (comboBox == DedupeDialog.this.comboBoxName1 || comboBox == DedupeDialog.this.comboBoxName2)
						if (DedupeDialog.this.rdbtnDedupeName.isSelected())
							DedupeDialog.this.btnRun.setEnabled(false);
