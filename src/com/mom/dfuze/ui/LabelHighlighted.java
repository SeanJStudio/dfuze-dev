...
public class LabelHighlighted extends JLabel {
    ...
    public void highlightText(String textToHighlight) {
        if (textToHighlight == null || textToHighlight.equalsIgnoreCase("^")) { // ^ causes fatal out-of-memory
            return;
        }
        
        textToHighlight = textToHighlight.replaceAll("\\*", "").replaceAll("\\|$", "");// * causes fatal out-of-memory
        ...
        Pattern SEARCH_PATTERN = Pattern.compile(textToHighlight, Pattern.CASE_INSENSITIVE);
        Matcher matcher = SEARCH_PATTERN.matcher(labelText);
        if (matcher.find()) {
