...
public class SimonFraserUniversity2 implements RunHarveyMckinnonAssociatesBehavior {
	...
	public void run(UserData userData) throws Exception {
		...
		for (int i = 0; i < data.length; ++i) {
			...
			nam1 = (fname + " " + lname).replaceAll("  ", " ").trim();
			...
			dearSal = fnameSal.replaceAll("  ", " ").trim();
			
			if(dearSal.length() == 0) {
				if(prefix.length() == 0)
					dearSal = "Friend";
				else
					dearSal = prefix + " " + lname;
			}
