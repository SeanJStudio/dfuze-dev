...
public class RegularProcess implements NewDemocraticPartyBehavior {
	...
	public void run(UserData userData) throws ApplicationException {
		...
		for (int i = 0; i < data.length; i++) {
			...
			String fullname = (fnam + " " + lnam).trim();
		...
		// Must create a new thread in order to use the dupe window
            SwingUtilities.invokeLater(new Runnable() {
                ...
                public void run() {
            		 ...
            		 * *********************/
            		for(Map.Entry<String, List<Record>> entry : dupeRecordsByPackageNum.entrySet()) {
            			String groupSize = entry.getValue().get(0).getPkgVer();
            			
            			String name = "";
            			String dearSal = "";
            			String lastname = entry.getValue().get(0).getLstName();
            				...
            				case "3":
        						for(int i = 0; i < entry.getValue().size(); ++i) {
        							if(i == entry.getValue().size() - 2) {
        								name += entry.getValue().get(i).getFstName() + " and ";
        								dearSal += entry.getValue().get(i).getFstName() + " and ";
        							} else if(i == entry.getValue().size() - 1) {
        								name += entry.getValue().get(i).getFstName();
        								dearSal += entry.getValue().get(i).getFstName();
        							} else {
        								name += entry.getValue().get(i).getFstName() + ", ";
        								dearSal += entry.getValue().get(i).getFstName() + ", ";
        							}
            			...
            			for(Map.Entry<String, List<Record>> entry : dupeRecordsByPackageNum.entrySet()) {
            				for(Record nextRecord : entry.getValue()) {
            					...
            					if(record.getDfId() == nextRecord.getDfId()) {
            						...
            						record.setDearSal(nextRecord.getDearSal());
            						record.setParaSal(nextRecord.getParaSal());
