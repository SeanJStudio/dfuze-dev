...
/**
 * NonAddressSeperatorFourLine implements a RunBehavior for 0 - Utility
 * 
 * @author Sean Johnson
 ...
 *
 */
public class NonAddressSeparatorFourLine implements RunUtilityBehavior {

	private final String BEHAVIOR_NAME = "4-Line Non-Address Separator";
	private String[] REQUIRED_FIELDS = {
			UserData.fieldName.ADDRESS1.getName(),
			UserData.fieldName.ADDRESS2.getName(),
			UserData.fieldName.ADDRESS3.getName(),
			UserData.fieldName.ADDRESS4.getName(),
			};
	...
	public void run(UserData userData) throws Exception {
		...
		userData.setDfHeaders(new String[] {
				...
				UserData.fieldName.COMPANY_ADDRESS4.getName(),
				UserData.fieldName.ADDRESS1.getName(),
				UserData.fieldName.ADDRESS2.getName(),
				UserData.fieldName.ADDRESS3.getName(),
				UserData.fieldName.ADDRESS4.getName(),
	...
	public void separateLines(UserData userData) throws Exception {
		...
		for(Record record : userData.getRecordList()) {
			...
			if(!MISC_ADDRESS_PATTERN.matcher(add4.replaceAll("\\.","")).find()) {
				if(add4.trim().isBlank() || add4.replaceAll("[^0-9]", "").isBlank()) {
					if(!hasStreetKeyword(add4, streetSuffixes, streetNames)) {
						if(record.getCmpnyAdd1().isBlank())
							record.setCmpnyAdd1(add4);
						else if(record.getCmpnyAdd2().isBlank())
							record.setCmpnyAdd2(add4);
						else if(record.getCmpnyAdd3().isBlank())
							record.setCmpnyAdd3(add4);
						else
							record.setCmpnyAdd4(add4);
	...
	public void organizeAddressLines(UserData userData) {
		for(Record record : userData.getRecordList()) {
			...
			if(!add4.isBlank()) {
				if(add3.isBlank() && !add1.isBlank() && !add2.isBlank()) {
					record.setAdd3(add4);
					record.setAdd4("");
				} else if (add2.isBlank() && !add1.isBlank()) {
					record.setAdd2(add4);
					record.setAdd4("");
				} else if (add1.isBlank()) {
					record.setAdd1(add4);
