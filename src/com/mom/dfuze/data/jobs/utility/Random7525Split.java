...
public class Random7525Split implements RunUtilityBehavior {

	private final String BEHAVIOR_NAME = "Random 75/25 Split";
	...
	Map<String, Integer> segmentCountMap;
	...
	public void run(UserData userData) throws ApplicationException {
		...
		int segSplitCount = (int) roundPercent((PERCENT * segmentCountMap.get(lastSeg)) / 100);
		...
		for(Record record: userData.getRecordList()) {
			...
			if(!record.getSeg().equals(lastSeg)) {
				...
				segSplitCount = (int) roundPercent((PERCENT * segmentCountMap.get(lastSeg)) / 100);
			}
			
			// If the current segment count is greater than 75%, change it to group2
