 ...
 *
 */
public class AdraUkraine implements RunGenerosityXBehavior {

	private final String BEHAVIOR_NAME = "Adra Ukraine";
	...
	public final Pattern GIFT_FILE_APPEAL_2022 = Pattern.compile("2022", Pattern.CASE_INSENSITIVE);
	public final Pattern GIFT_FILE_APPEAL_UKRAINE = Pattern.compile("ukraine", Pattern.CASE_INSENSITIVE);
	...
	public void run(UserData userData) throws Exception {
		...
		userData.setDfHeaders(new String[] {
				...
				UserData.fieldName.MONTH.getName(),
			...
			if(giftHistoryMap.containsKey(record.getInId())) {
				...
				for(int j = 0; j < giftHistoryList.size(); ++j) {
					...
					if(j == 0) {
						...
						// if the last appeal was a 2022 Ukraine appeal, flag it
						Matcher yearMatcher = GIFT_FILE_APPEAL_2022.matcher(giftHistory.getGiftCampaign());
						Matcher ukraineMatcher = GIFT_FILE_APPEAL_UKRAINE.matcher(giftHistory.getGiftCampaign());
						
						if(yearMatcher.find())
							if(ukraineMatcher.find())
								record.setAppeal("Ukraine");
