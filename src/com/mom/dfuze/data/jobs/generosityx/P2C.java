 ...
 *
 */
public class P2C implements RunGenerosityXBehavior {

	private final String BEHAVIOR_NAME = "P2C";
	...
	public void run(UserData userData) throws Exception {
		...
		HashMap<String, List<P2CGiftHistory>> giftHistoryMap = convertGiftsToMap(giftList, giftDateFormat);
	...
	private HashMap<String, List<P2CGiftHistory>> convertGiftsToMap(List<Record> giftList, String giftDateFormat) {

		DateTimeFormatter formatter = DateTimeFormatter.ofPattern(giftDateFormat);

		// id, giftHistory
		HashMap<String, List<P2CGiftHistory>> giftHistoryMap = new HashMap<>();
		...
		for(Record record : giftList) {
			...
			if(Validators.isStringOfDateFormat(record.getLstDnDat(), giftDateFormat)) {
				...
				P2CGiftHistory history = new P2CGiftHistory(
						record.getInId(),
						giftAmount,
						giftDate,
						record.getSeg()
				...
				if(!giftHistoryMap.containsKey(record.getInId()))
					giftHistoryMap.put(record.getInId(), new ArrayList<P2CGiftHistory>());
	...
	private void processGifts(UserData userData, HashMap<String, List<P2CGiftHistory>> giftHistoryMap) {
		final int MONTHS6 = 6;
		...
		for(int i = 0; i < userData.getRecordList().size(); ++i) {
			...
			if(giftHistoryMap.containsKey(record.getInId())) {
				List<P2CGiftHistory> giftHistoryList = giftHistoryMap.get(record.getInId());
				...
				for(int j = 0; j < giftHistoryList.size(); ++j) {
					P2CGiftHistory giftHistory = giftHistoryList.get(j);
				...
				// Set top donors
				if(largestGiftMadeLast18Months >= TOP_DONOR_AMOUNT)
